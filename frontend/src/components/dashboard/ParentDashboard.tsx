        ...
        <div className="animate-fade-in-up-delay relative overflow-hidden bg-gradient-to-br from-indigo-600 via-purple-600 to-violet-700 p-6 rounded-3xl shadow-xl text-white">
          ...
          <div className="relative z-10">
            <div className="flex items-center gap-2 mb-1">
              ...
              <h3 className="text-purple-200 text-sm font-bold">رصيد المحفظة الأساسية</h3>
            </div>
            <div className="mt-3 flex items-baseline gap-2">
              <span className="text-5xl font-black tracking-tight">{parentData.parent_balance || "0.00"}</span>
            ...
            <div className="mt-6 pt-4 border-t border-white/20">
              <h3 className="text-purple-200 text-xs font-bold mb-1">إجمالي الاستثمار في التعليم</h3>
              <div className="flex items-baseline gap-2">
                <span className="text-2xl font-black">{parentData.total_spent || "0.00"}</span>
            ...
            {/* محافظ الأبناء */}
            <div className="mt-6 pt-4 border-t border-white/20">
              <h4 className="text-xs font-extrabold mb-3 text-purple-200">أرصدة محافظ الأبناء:</h4>
              {parentData.wallets?.length === 0 ? (
                <p className="text-xs text-purple-300">لا يوجد أبناء مضافين بعد.</p>
              ) : (
                parentData.wallets?.map((w) => (
                  <div key={w.id} className="flex justify-between items-center text-sm mb-2 bg-white/10 backdrop-blur-sm p-2.5 rounded-xl border border-white/5">
                    ...
                    <span className="font-black">{w.balance} <span className="text-xs text-purple-200">ريال</span></span>
                  </div>
                ))
              )}
            </div>
      ...
      <div className="lg:col-span-2 animate-fade-in-up-delay bg-white/80 backdrop-blur-sm p-6 rounded-3xl shadow-lg border border-gray-100/80">
        ...
        {!parentData.bookings || parentData.bookings.length === 0 ? (
          <div className="text-center py-16">
            ...
            <h4 className="text-xl font-extrabold text-gray-800 mb-2">لا توجد حجوزات لأبنائك حتى الآن</h4>
            <p className="text-gray-400 text-sm">ابدأ بحجز حصص لأبنائك مع نخبة المعلمين</p>
          </div>
        ) : (
            ...
            <table className="w-full text-sm text-right">
              <thead>
                <tr className="bg-gradient-to-l from-gray-50 to-slate-50 border-b border-gray-200">
                  <th className="px-4 py-3.5 text-xs font-extrabold text-gray-500 uppercase tracking-wider rounded-tr-xl">الابن</th>
                  <th className="px-4 py-3.5 text-xs font-extrabold text-gray-500 uppercase tracking-wider">المعلم</th>
                  <th className="px-4 py-3.5 text-xs font-extrabold text-gray-500 uppercase tracking-wider">التاريخ والوقت</th>
                  <th className="px-4 py-3.5 text-xs font-extrabold text-gray-500 uppercase tracking-wider">التكلفة</th>
                  <th className="px-4 py-3.5 text-xs font-extrabold text-gray-500 uppercase tracking-wider rounded-tl-xl">الحالة</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                {parentData.bookings?.map((booking) => (
                  <tr key={booking.id} className="hover:bg-indigo-50/50 transition-all duration-200">
                    <td className="px-4 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="font-bold text-indigo-700">{booking.student?.name}</span>
                      </div>
                    </td>
                    <td className="px-4 py-4 font-bold text-gray-800">{booking.teacher?.name}</td>
                    <td className="px-4 py-4">
                      <div className="font-bold text-gray-800">{booking.booking_date?.substring(0, 10)}</div>
                      <div className="text-xs text-gray-400 mt-0.5">{booking.teacher_slot?.start_time?.substring(0, 5)}</div>
                    </td>
                    <td className="px-4 py-4">
