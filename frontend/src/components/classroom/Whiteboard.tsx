...
const Whiteboard: React.FC<WhiteboardProps> = React.memo(({ appIdentifier, roomUuid, roomToken, uid, isTeacher, region = 'eu' }) => {
    ...
    const applyTool = useCallback((tool: string, color?: string, width?: number) => {
        ...
        switch (tool) {
            ...
            case 'pencil': room.setMemberState({ currentApplianceName: ApplianceNames.pencil, strokeColor: hexToRgb(resolvedColor), strokeWidth: resolvedWidth }); break;
    ...
    const setWidth = useCallback((width: number) => { setStrokeWidth(width); if (activeTool !== 'selector' && activeTool !== 'eraser') applyTool(activeTool, strokeColor, width); }, [activeTool, strokeColor, applyTool]);
    const undo = useCallback(() => roomRef.current?.undo(), []);
    const redo = useCallback(() => roomRef.current?.redo(), []);
    const showToolbar = useCallback(() => { setToolbarVisible(true); if (hideTimerRef.current) clearTimeout(hideTimerRef.current); hideTimerRef.current = setTimeout(() => setToolbarVisible(false), 3000); }, []);
    ...
    const addPage = useCallback(() => { const room = roomRef.current; if (!room || !isTeacher) return; const newIndex = pageState.total; room.putScenes('/', [{}], newIndex); room.setScenePath(`/${newIndex}`); }, [isTeacher, pageState.total]);
    const goToPage = useCallback((index: number) => { const room = roomRef.current; if (!room || !isTeacher) return; if (index < 0 || index >= pageState.total) return; room.setScenePath(`/${index}`); }, [isTeacher, pageState.total]);
        ...
        <div className="w-full h-full flex flex-col bg-white rounded-xl overflow-hidden shadow-2xl relative">
            ...
            {error && (
                <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-red-50 p-6 text-center">
                    ...
                    <button onClick={() => window.location.reload()} className="bg-red-600 text-white px-6 py-2 rounded-lg font-bold hover:bg-red-700 transition">إعادة تحميل الصفحة</button>
                </div>
            )}
            {isTeacher && !loading && !error && (<div className="absolute top-0 left-0 right-0 h-4 z-50" onMouseEnter={showToolbar} />)}
            {isTeacher && !loading && !error && (
                <div className={`absolute top-3 left-1/2 -translate-x-1/2 z-40 w-max transition-all duration-300 ease-in-out ${toolbarVisible ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-full pointer-events-none'}`} onMouseEnter={showToolbar} onMouseLeave={() => { if (hideTimerRef.current) clearTimeout(hideTimerRef.current); hideTimerRef.current = setTimeout(() => setToolbarVisible(false), 3000); }}>
                    <div className="flex items-center gap-0.5 px-4 py-2 bg-slate-900/95 backdrop-blur-md rounded-2xl shadow-2xl border border-white/10">
                        ...
                        <div className="flex items-center gap-1 shrink-0">
                            {STROKE_COLORS.map(({ hex, label }) => (
                                <button key={hex} title={label} onClick={() => { setColor(hex); showToolbar(); }} className={`w-5 h-5 rounded-full border-2 transition-all hover:scale-110 active:scale-95 shrink-0 ${strokeColor === hex ? 'border-blue-400 scale-110 ring-2 ring-blue-400/40' : 'border-white/20'}`} style={{ backgroundColor: hex }} />
                            ))}
                        </div>
                        <div className="w-px h-6 bg-white/10 mx-1.5 shrink-0" />
                        <div className="flex items-center gap-0.5 shrink-0">
                            {STROKE_WIDTHS.map((w) => (
                                <button key={w} onClick={() => { setWidth(w); showToolbar(); }} title={`سُمك ${w}`} className={`w-9 h-8 rounded-lg flex items-center justify-center transition-all ${strokeWidth === w ? 'bg-blue-600/20 ring-1 ring-blue-500' : 'hover:bg-white/5'}`}><div className="rounded-full bg-white/80" style={{ width: 22, height: Math.max(1, Math.round(w * 0.55)) }} /></button>
                            ))}
                        </div>
                        <div className="w-px h-6 bg-white/10 mx-1.5 shrink-0" />
            ...
            {!loading && !error && (
                <div className="absolute bottom-3 left-1/2 -translate-x-1/2 z-40 flex items-center gap-2 bg-slate-900/90 backdrop-blur-md px-3 py-1.5 rounded-full shadow-xl border border-white/10">
                    {isTeacher && (<button onClick={() => goToPage(pageState.current - 1)} disabled={pageState.current === 0} className="p-1 text-slate-400 hover:text-white disabled:opacity-30 disabled:cursor-not-allowed transition"><ChevronRight size={16} /></button>)}
                    <span className="text-white text-xs font-bold min-w-[60px] text-center">{pageState.current + 1} / {pageState.total}</span>
